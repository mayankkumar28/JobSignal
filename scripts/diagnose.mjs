...
// Usage: npm run build && node scripts/diagnose.mjs
...
try {
  ...
  if (!fs.existsSync(path.join(EXT_PATH, 'manifest.json'))) {
    report.errors.push('FATAL: dist/manifest.json not found. Did npm run build succeed?');
  ...
  await page.screenshot({ path: 'diagnose-screenshot.png', fullPage: false });
  report.screenshot = 'diagnose-screenshot.png';
...
console.log('\n' + '='.repeat(70));
console.log('JOBSIGNAL DIAGNOSTIC REPORT');
